...
def db_import():
    ...
    try:
        ...
        # 데이터를 데이터베이스에 넣기
        final_df.to_sql(name='new_sijang_data', con=engine, if_exists='replace', index=False) # 새로운 db 데이터 
        print("새로운 db 데이터 넣기 성공.")
        final_df.to_sql(name='acc_sijang_data', con=engine, if_exists='append', index=False) # Append new data
        print("새로운 데이터 넣기 성공.")
        print('--------'*15)
        print(basic_df)
        print(country_df)
        ...
        print("새로운 db 데이터 넣기 성공.")
        country_df.to_sql(name='test2', con=engine, if_exists='replace', index=False) # Append new data
        print("새로운 데이터 넣기 성공.")
